...
export default function AppDropdownButton({
  ...
  return dropdownType == AppDropdownType.iconButton ? (
        ...
        <div className="absolute z-10 border border-gray-100 flex flex-col space-y-0 min-h-32 min-w-24 right-9  py-2 rounded  shadow-lg bg-white text-sm">
          {children.map((item) => (
            <div
              ...
              className="hover:bg-slate-100 px-2 py-1 text-center"
            >
              {item.title}
            </div>
          ))}
    ...
    <div className="cursor-pointer relative" ref={ref}>
      ...
      <div
        className="rounded border p-2 text-xs font-semibold shadow mr-4 mt-2 cursor-pointer hover:shadow-lg md:flex space-x-1 items-center transition-shadow duration-75"
        onClick={() => {
          toggleDropdown()
        }}
      >

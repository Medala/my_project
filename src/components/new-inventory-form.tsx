...
const NewInventoryForm = ({addInventory}:Props) => {
...
/*   const dropdownRef = useRef()
  ...
  return (
    <>
    {openFrom && <div onKeyDown={(e) => {e.code === "Escape" ? setOpen(false) : null}} className='fixed z-50 left-0 top-0 w-screen h-screen backdrop-blur-sm flex items-center justify-center'>
                    <div className='h-10/12 md:h-2/4 rounded-lg w-10/12 sm:w-2/4 bg-gray-300  overflow-auto shadow-lg'>

                     {/*  Form component title div */}
                      <div className="flex justify-center bg-gray-800 shadow-lg">
                        <div className="p-2 font-bold text-lg text-gray-400 text-grey-600">Create Inventory</div>
                      </div>
  ...
  <div className="w-full flex justify-between mt-4">
      <button onClick={() =>setOpen(false)} className="shadow bg-gray-800 hover:bg-gray-700 hover:text-white focus:shadow-outline focus:outline-none text-white font-bold py-2 px-4 rounded" type="button">
        Cancel
      </button>
      ...
      <button type="submit" className="shadow bg-gray-800 hover:bg-gray-700 focus:shadow-outline focus:outline-none text-white font-bold py-2 px-4 rounded">
        Save
              ...
              <div typeof="button" onClick={openForm} className='absolute z-50 cursor-pointer top-1 right-4 rounded-full h-10 w-10 hover bg-orange-500 hover:shadow-lg hover:h-12 hover:w-12 transition-all duration-100 flex items-center justify-center'>
